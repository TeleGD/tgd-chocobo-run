...
public class Collisions {
	...
	public static boolean collisionCircleAnyRect(Circle circle, Map map) {
		for(int i=0;i<map.walls.size();i++){
			if(collisionCircleRect(circle,map.walls.get(i))){
				return true;
			}
		}
		return false;
	...
	public static boolean collisionCircleAnyCircle(Circle circle, Map map) {
		for(int i=0;i<map.bonuses.size();i++){
			if(collisionCircleCircle(circle,map.bonuses.get(i))){
				return true;
			}
		}
		return false;
